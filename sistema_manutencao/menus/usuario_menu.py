...
class MenuUsuario:
    ...
    def buscar_usuario(self):
        ...
        if usuario is None:
            ...
            print(gradiente_texto(f"Nome........: {usuario.nome}", self.ROXO, self.LILAS_CLARO))
            print(gradiente_texto(f"E-mail......: {usuario.email}", self.LILAS_CLARO, self.ROXO_ESCURO))
    ...
    def _imprimir_lista(self, usuarios):
        ...
        for usuario in usuarios:
            print(
                gradiente_texto(
                    f"{usuario.id_usuario:<5}{usuario.nome:<25}{usuario.email:<30}{usuario.perfil_id:<8}",
    ...
    def atualizar_usuario(self):
        ...
        nome = input(
            gradiente_texto(f"Nome [{usuario.nome}]: ", self.ROXO_ESCURO, self.ROXO)
        )
        if nome:
            try:
                usuario.nome = validar_nome_usuario(nome)
            except ValueError as erro:
                print(gradiente_texto(f"Erro: {erro}", self.LILAS_CLARO, self.ROXO_ESCURO))
                input("\nPressione ENTER para continuar...")
                return
        ...
        email = input(
            gradiente_texto(f"E-mail [{usuario.email}]: ", self.ROXO, self.LILAS_CLARO)
        )
        if email:
            try:
                usuario.email = validar_email_usuario(email)
            except ValueError as erro:
                print(gradiente_texto(f"Erro: {erro}", self.LILAS_CLARO, self.ROXO_ESCURO))
                input("\nPressione ENTER para continuar...")
                return
