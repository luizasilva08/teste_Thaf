...
class MenuMaquina:
    ...
    def buscar_maquina(self):
        ...
        if maquina is None:
            ...
            print(gradiente_texto(f"Tag.................: {maquina.tag}", self.TEAL, self.TEAL_CLARO))
            print(gradiente_texto(f"Nome................: {maquina.nome}", self.TEAL_CLARO, self.TEAL_ESCURO))
    ...
    def _imprimir_lista(self, maquinas):
        ...
        for maquina in maquinas:
            print(
                gradiente_texto(
                    f"{maquina.id_maquina:<5}{maquina.tag:<12}{maquina.nome:<25}{maquina.setor_id:<8}{maquina.status_vivo:<12}",
    ...
    def atualizar_maquina(self):
        ...
        tag = input(
            gradiente_texto(f"Tag [{maquina.tag}]: ", self.TEAL_ESCURO, self.TEAL)
        )
        if tag:
            try:
                maquina.tag = validar_tag_maquina(tag)
            except ValueError as erro:
                print(gradiente_texto(f"Erro: {erro}", self.TEAL_CLARO, self.TEAL_ESCURO))
                input("\nPressione ENTER para continuar...")
                return
        ...
        nome = input(
            gradiente_texto(f"Nome [{maquina.nome}]: ", self.TEAL, self.TEAL_CLARO)
        )
        if nome:
            try:
                maquina.nome = validar_nome_maquina(nome)
            except ValueError as erro:
                print(gradiente_texto(f"Erro: {erro}", self.TEAL_CLARO, self.TEAL_ESCURO))
                input("\nPressione ENTER para continuar...")
                return
