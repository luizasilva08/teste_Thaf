...
class MenuTurma:
    ...
    def buscar_turma(self):
        ...
        if turma is None:
            ...
            print(gradiente_texto(f"Sigla.......: {turma.codigo}", self.VERDE, self.VERDE_CLARO))
            print(gradiente_texto(f"Período.....: {turma.periodo}", self.VERDE_CLARO, self.VERDE_ESCURO))
    ...
    def listar_turma(self):
        ...
        for turma in turmas:
            print(
                gradiente_texto(
                    f"{turma.id_turma:<5}{turma.codigo:<20}{turma.periodo:<25}",
    ...
    def atualizar_turma(self):
        ...
        codigo = input(
            gradiente_texto(f"Código [{turma.codigo}]: ", self.VERDE_ESCURO, self.VERDE)
        )
        if codigo:
            try:
                turma.codigo = validar_codigo_turma(codigo)
            except ValueError as erro:
                print(gradiente_texto(f"Erro: {erro}", self.VERDE_CLARO, self.VERDE_ESCURO))
                input("\nPressione ENTER para continuar...")
                return
        ...
        periodo = input(
            gradiente_texto(f"Período [{turma.periodo}]: ", self.VERDE, self.VERDE_CLARO)
        )
        if periodo:
